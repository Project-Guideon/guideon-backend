...
CREATE TABLE IF NOT EXISTS tb_site (
  ...
  -- Admin 지도 페이지 초기 중심점 (표시용, 거리 계산 안 함 → geography 불필요)
  latitude    DOUBLE PRECISION NULL,
  longitude   DOUBLE PRECISION NULL,
  map_level   SMALLINT NULL,   -- 카카오맵 줌 레벨(1~14)
  created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- 기존 DB 마이그레이션 (이미 생성된 tb_site에 컬럼 보강)
ALTER TABLE tb_site ADD COLUMN IF NOT EXISTS latitude  DOUBLE PRECISION NULL;
ALTER TABLE tb_site ADD COLUMN IF NOT EXISTS longitude DOUBLE PRECISION NULL;
ALTER TABLE tb_site ADD COLUMN IF NOT EXISTS map_level SMALLINT NULL;
...
CREATE TABLE IF NOT EXISTS tb_admin (
  ...
  role           VARCHAR(20) NOT NULL DEFAULT 'SITE_ADMIN',
  is_active      BOOLEAN NOT NULL DEFAULT TRUE,
...
CREATE TABLE IF NOT EXISTS tb_place (
  ...
  site_id      BIGINT NOT NULL REFERENCES tb_site(site_id) ON DELETE CASCADE,
  zone_id      BIGINT NULL,
  zone_source  VARCHAR(10) NOT NULL DEFAULT 'AUTO',
  name         VARCHAR(100) NOT NULL,
...
-- tb_mascot_generation (Tripo AI 3D 생성 task 추적)
CREATE TABLE IF NOT EXISTS tb_mascot_generation (
  generation_id      BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  site_id            BIGINT NOT NULL REFERENCES tb_site(site_id) ON DELETE CASCADE,
  source_image_url   VARCHAR(500) NOT NULL,

  -- Tripo API task 추적
  model_task_id      VARCHAR(100) NULL,
...
CREATE INDEX IF NOT EXISTS idx_chat_session_device ON tb_chat_session (device_id);
CREATE INDEX IF NOT EXISTS idx_chat_session_site ON tb_chat_session (site_id);

-- tb_chat_message
CREATE TABLE IF NOT EXISTS tb_chat_message (
  message_id      BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  session_id      VARCHAR(36) NOT NULL REFERENCES tb_chat_session(session_id) ON DELETE CASCADE,
  site_id         BIGINT NOT NULL REFERENCES tb_site(site_id) ON DELETE CASCADE,
  device_id       VARCHAR(50) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_chat_msg_session ON tb_chat_message (session_id);
CREATE INDEX IF NOT EXISTS idx_chat_msg_site_created ON tb_chat_message (site_id, created_at);
CREATE INDEX IF NOT EXISTS idx_chat_msg_category ON tb_chat_message (site_id, category);
...
CREATE INDEX IF NOT EXISTS idx_pairing_status_expires ON tb_pairing_request (status, expires_at);

...
public class StatsController {
    ...
    public ResponseEntity<ApiResponse<QuestionTypeStatDto>> getQuestionTypeStats(
            @PathVariable Long siteId, HttpServletRequest httpRequest) {
        String traceId = (String) httpRequest.getAttribute(TraceIdUtil.TRACE_ID_ATTR);
        return ResponseEntity.ok(ApiResponse.success(coreStatsClient.getQuestionTypeStats(siteId), traceId));
    }
    ...
    public ResponseEntity<ApiResponse<AnswerRateStatDto>> getAnswerRateStat(
            @PathVariable Long siteId, HttpServletRequest httpRequest) {
        String traceId = (String) httpRequest.getAttribute(TraceIdUtil.TRACE_ID_ATTR);
        return ResponseEntity.ok(ApiResponse.success(coreStatsClient.getAnswerRateStat(siteId), traceId));
    }
    ...
    public ResponseEntity<ApiResponse<HourlyTrafficStatDto>> getHourlyTrafficStat(
            @PathVariable Long siteId, HttpServletRequest httpRequest) {
        String traceId = (String) httpRequest.getAttribute(TraceIdUtil.TRACE_ID_ATTR);
        return ResponseEntity.ok(ApiResponse.success(coreStatsClient.getHourlyTrafficStat(siteId), traceId));
    }
    ...
    public ResponseEntity<ApiResponse<SiteTrafficTop5Dto>> getSiteTrafficTop5(
        ...
        String traceId = (String) httpRequest.getAttribute(TraceIdUtil.TRACE_ID_ATTR);
        return ResponseEntity.ok(ApiResponse.success(coreStatsClient.getSiteTrafficTop5(), traceId));
    }
    ...
    public ResponseEntity<ApiResponse<DeviceStatusStatDto>> getDeviceStatusStat(
            @PathVariable Long siteId, HttpServletRequest httpRequest) {
        String traceId = (String) httpRequest.getAttribute(TraceIdUtil.TRACE_ID_ATTR);
        return ResponseEntity.ok(ApiResponse.success(coreStatsClient.getDeviceStatusStat(siteId), traceId));
    }
}
